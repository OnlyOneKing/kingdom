...
export default class Zuimeihaveson extends React.Component {
	constructor (props) {
		super(props)
		this.state = {
			need_pic_desc:0,
			...
			sliderOffset: Store.getInstance().zuimeiHaveson.sliderOffset,
			index: Store.getInstance().zuimeiHaveson.index ? Store.getInstance().zuimeiHaveson.index : 0,
			currentlist: Store.getInstance().zuimeiHaveson.list ? Store.getInstance().zuimeiHaveson.list[Store.getInstance().zuimeiHaveson.index] : [],
			list: Store.getInstance().zuimeiHaveson.list,
			tabs: Store.getInstance().zuimeiHaveson.tabs,
			dataList: Store.getInstance().zuimeiHaveson.dataList, // 子分组数据列表
			picList: Store.getInstance().zuimeiHaveson.picList, // pic数据列表
			picHeigth:Store.getInstance().zuimeiHaveson.picHeigth, // 最受关注数据高度 0左侧数据 1右侧数据
			sonScroll: Store.getInstance().zuimeiHaveson.sonScroll, // 选项卡的滚动高度, 0 全部分组 1最新作品 2大赛说明
			limit: 6, // 子分组数据分页，每页显示几条数据
			page: Store.getInstance().zuimeiHaveson.page, // 子分组数据分页页码 0 全部分组 1最新作品
			type: 1, // 子分组数据类型 1全部分组 2最新作品 3大赛说明
			status: Store.getInstance().zuimeiHaveson.status,  // 是否能继续加载 0 全部分组 1最新作品
			id: 0, // 大赛id
			province: '北京', // 最美省市
			loading: false,
			...
			toggleNav: Store.getInstance().zuimeiHaveson.toggleNav, // 切换选项卡

		}
		this.timer = ''
	}
	componentWillMount () {
		console.log('Store.getInstance().zuimeiHaveson', Store.getInstance().zuimeiHaveson)
	}
	componentDidUpdate () {
		if (this.state.toggleNav) {
			$('body,html').animate({scrollTop: this.state.sonScroll[this.state.index]}, 0)
			this.state.toggleNav = false
			Store.getInstance().zuimeiHaveson.toggleNav = false
		}
	...
	initPage = (props) => {
		...
		this.state.id = id
		let type = 1
		if (Store.getInstance().zuimeiHaveson.index) {
			type = parseFloat(Store.getInstance().zuimeiHaveson.index + 1)
		}
		...
		Service.getDetailsFocusList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					this.state.province = resp.data.province;
	...
	onScroll = (e) => {
		...
		Store.getInstance().zuimeiHaveson.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getSonActivityList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().zuimeiHaveson.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getSonActivityList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().zuimeiHaveson.dataList = dataList
					Store.getInstance().zuimeiHaveson.page[0] = resp.data.next_page
					Store.getInstance().zuimeiHaveson.status = status
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().zuimeiHaveson.picList = picList
					Store.getInstance().zuimeiHaveson.picHeigth = picHeigth
					Store.getInstance().zuimeiHaveson.page[1] = resp.data.next_page
					Store.getInstance().zuimeiHaveson.status = status
					_this.setState({
						picList,
						picHeigth,
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().zuimeiHaveson.index = index
		Store.getInstance().zuimeiHaveson.sliderOffset = sliderOffset
	
		this.setState({
			sliderOffset:Store.getInstance().zuimeiHaveson.sliderOffset,
			index:Store.getInstance().zuimeiHaveson.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().zuimeiHaveson.toggleNav = true
		hashHistory.push({
			pathname: 'details',
			query: {
				rid
                            ...
                            this.state.focuslist.map((item,index) => {
                                return (
                                    <Slide className="_swiper__slide" key={index} >
                                    	...
                                    	<div className='goto-map flex flex-ver-center' onClick={this.gotoMap}>
                                    		<img src="img/map_lbs.png" />
                                    		<span>观景点地图</span>
                                    	</div>
                                    </Slide>
                                )
