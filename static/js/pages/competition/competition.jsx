...
export default class Competition extends React.Component {
	constructor (props) {
		...
		this.state = {
			focuslist: [], // 焦点图数据
			sliderOffset: Store.getInstance().sindex.competition.sliderOffset,
			index: Store.getInstance().sindex.competition.index ? Store.getInstance().sindex.competition.index : 0,
			currentlist: Store.getInstance().sindex.competition.list.length ? Store.getInstance().sindex.competition.list[Store.getInstance().sindex.competition.index] : [],
			list: Store.getInstance().sindex.competition.list,
			tabs: Store.getInstance().sindex.competition.tabs,
			dataList: Store.getInstance().sindex.competition.dataList, // 进行中数据列表
			picList: Store.getInstance().sindex.competition.picList, // 已结束数据列表
			limit: 5, // 大赛数据分页，每页显示几条数据
			page: Store.getInstance().sindex.competition.page, // 大赛数据分页页码
			sonScroll: Store.getInstance().sindex.competition.sonScroll, // 选项卡的滚动高度, 0 进行中 1 已结束
			type: 1, // 大赛数据类型 1进行中 2已结束
			status: Store.getInstance().sindex.competition.status,  // 是否能继续加载
			client_time: Date.parse(new Date()) / 1000,
			serve_time: 0,
			navbarTop: 0,
			loading: false,
			loadingData: [true, true], // 函数节流，防止多次加载
			toggleNav: Store.getInstance().sindex.competition.toggleNav, // 切换选项卡
		}
		this.listener = [] // 倒计时定时器
	}
	componentWillMount () {
		document.title = '摄影大赛首页'
		this._loadConfig()
	}
	componentDidUpdate () {
		if (this.state.toggleNav) {
			...
			Store.getInstance().sindex.competition.toggleNav = false
		}
	}
	componentDidMount () {
		let type = 1
		if (Store.getInstance().sindex.competition.index) {
			type = parseFloat(Store.getInstance().sindex.competition.index + 1)
		}
	...
	onScroll = (e) => {
		...
		Store.getInstance().sindex.competition.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getDataList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().sindex.competition.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getDataList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().sindex.competition.dataList = dataList
					Store.getInstance().sindex.competition.page[0] = resp.data.next_page
					Store.getInstance().sindex.competition.status = status
		...
		Service.getDataList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().sindex.competition.picList = picList
					Store.getInstance().sindex.competition.page[1] = resp.data.next_page
					Store.getInstance().sindex.competition.status = status
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().sindex.competition.index = index
		Store.getInstance().sindex.competition.sliderOffset = sliderOffset
		this.setState({
			sliderOffset:Store.getInstance().sindex.competition.sliderOffset,
			index:Store.getInstance().sindex.competition.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().sindex.competition.toggleNav = true
		Store.getInstance().sindex.competnoson = this.cloneObjectFn(Store.getInstance().sindex.competnosonCopy)
		Store.getInstance().sindex.compethaveson = this.cloneObjectFn(Store.getInstance().sindex.compethavesonCopy)
		let url = son == 1 ? 'compethaveson' : 'competnoson'
		hashHistory.push({
			pathname: url,
			query: {
				id,
