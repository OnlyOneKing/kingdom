...
export default class Compethaveson extends React.Component {
	constructor (props) {
		super(props)
		this.state = {
			need_pic_desc:0,
			...
			sliderOffset: Store.getInstance().sindex.compethaveson.sliderOffset,
			index: Store.getInstance().sindex.compethaveson.index ? Store.getInstance().sindex.compethaveson.index : 0,
			currentlist: Store.getInstance().sindex.compethaveson.list.length ? Store.getInstance().sindex.compethaveson.list[Store.getInstance().sindex.compethaveson.index] : [],
			list: Store.getInstance().sindex.compethaveson.list,
			tabs: Store.getInstance().sindex.compethaveson.tabs,
			dataList: Store.getInstance().sindex.compethaveson.dataList, // 子分组数据列表
			picList: Store.getInstance().sindex.compethaveson.picList, // pic数据列表
			picHeigth:Store.getInstance().sindex.compethaveson.picHeigth, // 最受关注数据高度 0左侧数据 1右侧数据
			sonScroll: Store.getInstance().sindex.compethaveson.sonScroll, // 选项卡的滚动高度, 0 全部分组 1最新作品 2大赛说明
			limit: 6, // 子分组数据分页，每页显示几条数据
			page: Store.getInstance().sindex.compethaveson.page, // 子分组数据分页页码 0 全部分组 1最新作品
			type: 1, // 子分组数据类型 1全部分组 2最新作品 3大赛说明
			status: Store.getInstance().sindex.compethaveson.status,  // 是否能继续加载 0 全部分组 1最新作品
			id: 0, // 大赛id
			loading: false,
			navbarTop: 0, // 选项卡距离顶部高度
			title: '', // 大赛标题
			loadingData: [true, true], // 函数节流，防止多次加载
			toggleNav: Store.getInstance().sindex.compethaveson.toggleNav, // 切换选项卡
	...
	componentDidUpdate () {
		if (this.state.toggleNav) {
			...
			Store.getInstance().sindex.compethaveson.toggleNav = false
		}
	...
	initPage = (props) => {
		...
		this.state.id = id
		let type = 1
		if (Store.getInstance().sindex.compethaveson.index) {
			type = parseFloat(Store.getInstance().sindex.compethaveson.index + 1)
		}
	...
	setShare = () => {
		...
		let sharedes = this.state.desc?this.state.desc:'来自「中国国家地理图片」';//
	...
	onScroll = (e) => {
		...
		Store.getInstance().sindex.compethaveson.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getSonActivityList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().sindex.compethaveson.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getSonActivityList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().sindex.compethaveson.dataList = dataList
					Store.getInstance().sindex.compethaveson.page[0] = resp.data.next_page
					Store.getInstance().sindex.compethaveson.status = status
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().sindex.compethaveson.picList = picList
					Store.getInstance().sindex.compethaveson.picHeigth = picHeigth
					Store.getInstance().sindex.compethaveson.page[1] = resp.data.next_page
					Store.getInstance().sindex.compethaveson.status = status
					_this.setState({
						picList,
						picHeigth,
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().sindex.compethaveson.index = index
		Store.getInstance().sindex.compethaveson.sliderOffset = sliderOffset
	
		this.setState({
			sliderOffset:Store.getInstance().sindex.compethaveson.sliderOffset,
			index:Store.getInstance().sindex.compethaveson.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().sindex.compethaveson.toggleNav = true
		hashHistory.push({
			pathname: 'details',
			query: {
				rid
	...
	gotoNext = (e) => {
		let id = e.currentTarget.getAttribute("data-id")
		Store.getInstance().sindex.competnoson = this.cloneObjectFn(Store.getInstance().sindex.competnosonCopy)
		hashHistory.push({
			pathname: 'competnoson',
			query: {
				id
			}
		})
	...
	render () {
		return (
			<div className='index-content competition-content compethaveson-content'>
			{/*轮播图*/}
				{
									...
									return (
												<div className='lists-item' key={index}>
													...
													<div className='item-con' data-id={item.id} onClick={this.gotoNext.bind(this)}>
														<h1>{item.title}</h1>
														<div className='data compethaveson-data flex flex-ver-center'>
													...
													<div className='compethaveson-bottom flex flex-space-around flex-ver-center'>
															...
															<div className='bottom-btn green' data-title={item.title} data-id={item.id} onClick={this.join.bind(this)}>参加本组比赛</div>
													</div>
												</div> 
											)
										})
				...
				<div>
						{
							...
							<div className='nodata'>{
								this.state.loading ? '加载中' : this.state.picList[0].length ? '加载中' : 
								(
									<div className="coming-soon flex-column flex-ver-center">
										<div style={{fontSize:'17px',marginTop:'50px'}}>作品征集中...</div>
									</div>
