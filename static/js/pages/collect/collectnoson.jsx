...
export default class Collectnoson extends React.Component {
	constructor (props) {
		super(props)
		this.state = {
			need_pic_desc:0,
			need_pic_position:0,
			allow_media_type: 1, 
			zuimeiDetail: '', // type 3 时为最美拍摄点的数据
			focuslist: [], // 焦点图数据
			content: '', // 大赛说明
			desc:'',
			tips:'',
			
			sliderOffset: Store.getInstance().sindex.collectnoson.sliderOffset,
			index: Store.getInstance().sindex.collectnoson.index ? Store.getInstance().sindex.collectnoson.index : 0,
			currentlist: Store.getInstance().sindex.collectnoson.list.length ? Store.getInstance().sindex.collectnoson.list[Store.getInstance().sindex.collectnoson.index] : [],
			list: Store.getInstance().sindex.collectnoson.list,
			tabs: Store.getInstance().sindex.collectnoson.tabs,
			dataList: Store.getInstance().sindex.collectnoson.dataList, // 最新发布数据列表 0左侧数据 1右侧数据
			dataHeigth:Store.getInstance().sindex.collectnoson.dataHeigth, // 最新发布数据高度 0左侧数据 1右侧数据
			picList: Store.getInstance().sindex.collectnoson.picList, // 最受关注数据列表 0左侧数据 1右侧数据
			picHeigth:Store.getInstance().sindex.collectnoson.picHeigth, // 最受关注数据高度 0左侧数据 1右侧数据
			sonScroll: Store.getInstance().sindex.collectnoson.sonScroll, // 选项卡的滚动高度, 0 最新发布 1最受关注 2大赛说明
			limit: 6, // 子分组数据分页，每页显示几条数据
			page: Store.getInstance().sindex.collectnoson.page, // 数据分页页码 0 最新发布 1最受关注
			type: 1, // 子分组数据类型 1 最新发布 2最受关注 3大赛说明
			status: Store.getInstance().sindex.collectnoson.status,  // 是否能继续加载 0 最新发布 1最受关注
			id: 0, // 大赛id
			loading: false,
			navbarTop: Store.getInstance().sindex.collectnoson.navbarTop, // 选项卡距离顶部高度
			title: '', // 大赛标题
			bg_img: '',
			son: false , // 是否有子集 
			loadingData: [true, true], // 函数节流，防止多次加载
			toggleNav: Store.getInstance().sindex.collectnoson.toggleNav, // 切换选项卡
		}
		this.timer = ''
	}
	componentWillMount () {
		console.log('dataList', Store.getInstance().sindex.collectnoson.dataList)
	}
	componentDidUpdate () {
		if (this.state.toggleNav) {
			$('body,html').animate({scrollTop: this.state.sonScroll[this.state.index]}, 0)
			this.state.toggleNav = false
			Store.getInstance().sindex.collectnoson.toggleNav = false
		}
		// 对大赛说明进行样式调整
	...
	initPage = (props) => {
		...
		let type = 1
		if (Store.getInstance().sindex.collectnoson.index) {
			type = parseFloat(Store.getInstance().sindex.collectnoson.index + 1)
		}
	...
	setShare = () => {
		...
		} else if (this.state.focuslist.length > 0) {
			...
			switch (resp.code) {
				case 0:
					...
					if (resp.data.type == 3) {
						...
						if (!zuimeiDetail.focus && data) {
							zuimeiDetail.focus = data[0]
						}
						//强制仅使用详情图
						zuimeiDetail.focus = resp.data.focus1[0];

						_this.setState({
							zuimeiDetail,
							tabs
						})
					} else if (!data && resp.data.focus) {
						data = [resp.data.focus]
					}
					//强制仅使用详情图
					data = [resp.data.focus1]
					document.title = title
					
					if (!Store.getInstance().gotoDetailYe) {
						$('.navBarItem')[2].click()
					}

					this.state.title = title
					this.state.focuslist = data
					this.state.desc = resp.data.desc
					this.state.bg_img = resp.data.bg_img
	...
	onScroll = (e) => {
		...
		} else if ($('.zuimeiBanner').outerHeight()) {
			this.state.navbarTop = $('.zuimeiBanner').outerHeight()
		} else {
		...
		Store.getInstance().sindex.collectnoson.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getNewList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().sindex.collectnoson.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().sindex.collectnoson.dataList = dataList
					Store.getInstance().sindex.collectnoson.dataHeigth = dataHeigth
					Store.getInstance().sindex.collectnoson.page[0] = resp.data.next_page
					Store.getInstance().sindex.collectnoson.status = status
					_this.setState({
						dataList,
						dataHeigth,
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().sindex.collectnoson.picList = picList
					Store.getInstance().sindex.collectnoson.picHeigth = picHeigth
					Store.getInstance().sindex.collectnoson.page[1] = resp.data.next_page
					Store.getInstance().sindex.collectnoson.status = status
					_this.setState({
						picList,
						picHeigth,
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().sindex.collectnoson.index = index
		Store.getInstance().sindex.collectnoson.sliderOffset = sliderOffset
		this.setState({
			sliderOffset:Store.getInstance().sindex.collectnoson.sliderOffset,
			index:Store.getInstance().sindex.collectnoson.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().sindex.collectnoson.toggleNav = true
		Store.getInstance().gotoDetailYe = true
		hashHistory.push({
			pathname: 'details',
			query: {
				rid
	...
	render () {
		return (
			...
			{/*轮播图*/}
				{
					this.state.focuslist.length > 0 && this.state.focuslist[0] ? 
					<Swiper
                        ref="swiperObj"
                        swiperOptions={{
                          slidesPerView: 'auto'
                    ...
                    <div 
                    	...
                    	style={{backgroundImage:'url(' + this.state.zuimeiDetail.focus + '?x-oss-process=image/resize,w_' + Store.getInstance().clientWidth + ')', height: Store.getInstance().clientWidth*9/16 + 'px'}}>
                    	<div className='zuimeiMain'>
	                    	<div className='zuimeiContent flex flex-space-between'>
	        ...
	        {/*tab选择*/}
	        	<div className='bar-body'>
	        	{
					...
					/> : <div className='navBar-replace'>征集说明</div>
	        	}
	        	</div>
			{/*主题内容列表*/}
				{
					...
					<div>
						...
						<div className='nodata'>{
							this.state.loading ? '加载中' : this.state.dataList.length[0] > 0 ? '加载中' : 
							(
				...
				:
				<div className='shuoming shuoming-collect' 
					 style={{backgroundImage: 'url(' + this.state.bg_img + '?x-oss-process=image/resize,w_' + Store.getInstance().clientWidth + ')'}}>
					{
			...
			{/*参赛按钮*/}
				<div className='join-active flex'>
					{
						this.state.son ?
						<div className='back-index' onClick={this.gotoNext}>全部征集</div> : ''
					}
					<div className='join-go' data-title={this.state.title} data-id={this.state.id} onClick={this.join.bind(this)}>立即投稿</div>
				</div>
