...
export default class Collect extends React.Component {
	constructor (props) {
		...
		this.state = {
			focuslist: [], // 焦点图数据
			sliderOffset: Store.getInstance().sindex.collect.sliderOffset,
			index: Store.getInstance().sindex.collect.index ? Store.getInstance().sindex.collect.index : 0,
			currentlist: Store.getInstance().sindex.collect.list.length ? Store.getInstance().sindex.collect.list[Store.getInstance().sindex.collect.index] : [],
			list: Store.getInstance().sindex.collect.list,
			tabs: Store.getInstance().sindex.collect.tabs,
			dataList: Store.getInstance().sindex.collect.dataList, // 进行中数据列表
			picList: Store.getInstance().sindex.collect.picList, // 已结束数据列表
			limit: 5, // 大赛数据分页，每页显示几条数据
			page: Store.getInstance().sindex.collect.page, // 大赛数据分页页码
			sonScroll: Store.getInstance().sindex.collect.sonScroll, // 选项卡的滚动高度, 0 进行中 1 已结束
			type: 1, // 大赛数据类型 1进行中 2已结束
			status: Store.getInstance().sindex.collect.status,  // 是否能继续加载
			client_time: Date.parse(new Date()) / 1000,
			serve_time: 0,
			navbarTop: 0,
			loading: false,
			loadingData: [true, true], // 函数节流，防止多次加载
			toggleNav: Store.getInstance().sindex.collect.toggleNav, // 切换选项卡
		}
	...
	componentDidUpdate () {
		if (this.state.toggleNav) {
			...
			Store.getInstance().sindex.collect.toggleNav = false
		}
	}
	componentDidMount () {
		let type = 1
		if (Store.getInstance().sindex.collect.index) {
			type = parseFloat(Store.getInstance().sindex.collect.index + 1)
		}
	...
	onScroll = (e) => {
		...
		Store.getInstance().sindex.collect.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getDataList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().sindex.collect.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getDataList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().sindex.collect.dataList = dataList
					Store.getInstance().sindex.collect.page[0] = resp.data.next_page
					Store.getInstance().sindex.collect.status = status
		...
		Service.getDataList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().sindex.collect.picList = picList
					Store.getInstance().sindex.collect.page[1] = resp.data.next_page
					Store.getInstance().sindex.collect.status = status
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().sindex.collect.index = index
		Store.getInstance().sindex.collect.sliderOffset = sliderOffset
		this.setState({
			sliderOffset:Store.getInstance().sindex.collect.sliderOffset,
			index:Store.getInstance().sindex.collect.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().sindex.collect.toggleNav = true
		Store.getInstance().sindex.collectnoson = this.cloneObjectFn(Store.getInstance().sindex.collectnosonCopy)
		Store.getInstance().sindex.collecthaveson = this.cloneObjectFn(Store.getInstance().sindex.collecthavesonCopy)
		let url = son == 1 ? 'collecthaveson' : 'collectnoson'
		hashHistory.push({
			pathname: url,
			query: {
				id,
