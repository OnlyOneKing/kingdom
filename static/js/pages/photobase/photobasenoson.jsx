...
export default class Photobasenoson extends React.Component {
	constructor (props) {
		super(props)
		this.state = {
			need_pic_desc:0,
			...
			sliderOffset: Store.getInstance().photobaseNoson.sliderOffset,
			index: Store.getInstance().photobaseNoson.index ? Store.getInstance().photobaseNoson.index : 0,
			currentlist: Store.getInstance().photobaseNoson.list.length ? Store.getInstance().photobaseNoson.list[Store.getInstance().photobaseNoson.index] : [],
			list: Store.getInstance().photobaseNoson.list,
			tabs: Store.getInstance().photobaseNoson.tabs,
			dataList: Store.getInstance().photobaseNoson.dataList, // 最新发布数据列表 0左侧数据 1右侧数据
			dataHeigth:Store.getInstance().photobaseNoson.dataHeigth, // 最新发布数据高度 0左侧数据 1右侧数据
			picList: Store.getInstance().photobaseNoson.picList, // 最受关注数据列表 0左侧数据 1右侧数据
			picHeigth:Store.getInstance().photobaseNoson.picHeigth, // 最受关注数据高度 0左侧数据 1右侧数据
			sonScroll: Store.getInstance().photobaseNoson.sonScroll, // 选项卡的滚动高度, 0 最新发布 1最受关注 2大赛说明
			limit: 6, // 子分组数据分页，每页显示几条数据
			page: Store.getInstance().photobaseNoson.page, // 数据分页页码 0 最新发布 1最受关注
			type: 1, // 子分组数据类型 1 最新发布 2最受关注 3大赛说明
			status: Store.getInstance().photobaseNoson.status,  // 是否能继续加载 0 最新发布 1最受关注
			id: 0, // 大赛id
			loading: false,
			navbarTop: Store.getInstance().photobaseNoson.navbarTop, // 选项卡距离顶部高度
			title: '', // 大赛标题
			bg_img: '',
			son: false , // 是否有子集 
			loadingData: [true, true], // 函数节流，防止多次加载
			toggleNav: Store.getInstance().photobaseNoson.toggleNav, // 切换选项卡
		}
		this.timer = ''
	}
	componentWillMount () {
		console.log('dataList', Store.getInstance().photobaseNoson)
	}
	componentDidUpdate () {
		if (this.state.toggleNav) {
			$('body,html').animate({scrollTop: this.state.sonScroll[this.state.index]}, 0)
			this.state.toggleNav = false
			Store.getInstance().photobaseNoson.toggleNav = false
		}
		// 对大赛说明进行样式调整
	...
	initPage = (props) => {
		...
		let type = 1
		if (Store.getInstance().photobaseNoson.index) {
			type = parseFloat(Store.getInstance().photobaseNoson.index + 1)
		}
		...
		Service.getDetailsFocusList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					console.log('Store.getInstance().collectnoson', Store.getInstance().collectnoson)
					...
					console.log('resp.data.bg_img', resp.data.bg_img)
					this.setShare()
					_this.setState({
						tips:resp.data.tips,
	...
	onScroll = (e) => {
		...
		Store.getInstance().photobaseNoson.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getNewList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().photobaseNoson.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().photobaseNoson.dataList = dataList
					Store.getInstance().photobaseNoson.dataHeigth = dataHeigth
					Store.getInstance().photobaseNoson.page[0] = resp.data.next_page
					Store.getInstance().photobaseNoson.status = status
					_this.setState({
						dataList,
						dataHeigth,
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().photobaseNoson.picList = picList
					Store.getInstance().photobaseNoson.picHeigth = picHeigth
					Store.getInstance().photobaseNoson.page[1] = resp.data.next_page
					Store.getInstance().photobaseNoson.status = status
					_this.setState({
						picList,
						picHeigth,
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().photobaseNoson.index = index
		Store.getInstance().photobaseNoson.sliderOffset = sliderOffset
		this.setState({
			sliderOffset:Store.getInstance().photobaseNoson.sliderOffset,
			index:Store.getInstance().photobaseNoson.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().photobaseNoson.toggleNav = true
