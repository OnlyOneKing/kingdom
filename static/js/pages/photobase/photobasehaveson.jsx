...
export default class Photobasehaveson extends React.Component {
	constructor (props) {
		super(props)
		this.state = {
			need_pic_desc:0,
			...
			sliderOffset: Store.getInstance().photobaseHaveson.sliderOffset,
			index: Store.getInstance().photobaseHaveson.index ? Store.getInstance().photobaseHaveson.index : 0,
			currentlist: Store.getInstance().photobaseHaveson.list.length ? Store.getInstance().photobaseHaveson.list[Store.getInstance().photobaseHaveson.index] : [],
			list: Store.getInstance().photobaseHaveson.list,
			tabs: Store.getInstance().photobaseHaveson.tabs,
			dataList: Store.getInstance().photobaseHaveson.dataList, // 子分组数据列表
			picList: Store.getInstance().photobaseHaveson.picList, // pic数据列表
			picHeigth:Store.getInstance().photobaseHaveson.picHeigth, // 最受关注数据高度 0左侧数据 1右侧数据
			sonScroll: Store.getInstance().photobaseHaveson.sonScroll, // 选项卡的滚动高度, 0 全部分组 1最新作品 2大赛说明
			limit: 6, // 子分组数据分页，每页显示几条数据
			page: Store.getInstance().photobaseHaveson.page, // 子分组数据分页页码 0 全部分组 1最新作品
			type: 1, // 子分组数据类型 1全部分组 2最新作品 3大赛说明
			status: Store.getInstance().photobaseHaveson.status,  // 是否能继续加载 0 全部分组 1最新作品
			id: 0, // 大赛id
			loading: false,
			navbarTop: Store.getInstance().photobaseHaveson.navbarTop, // 选项卡距离顶部高度
			title: '', // 大赛标题
			loadingData: [true, true], // 函数节流，防止多次加载
			toggleNav: Store.getInstance().photobaseHaveson.toggleNav, // 切换选项卡
	...
	componentDidUpdate () {
		if (this.state.toggleNav) {
			...
			Store.getInstance().photobaseHaveson.toggleNav = false
		}
		// 对大赛说明进行样式调整
	...
	initPage = (props) => {
		...
		this.state.id = id
		let type = 1
		if (Store.getInstance().photobaseHaveson.index) {
			type = parseFloat(Store.getInstance().photobaseHaveson.index + 1)
		}
	...
	onScroll = (e) => {
		...
		Store.getInstance().photobaseHaveson.sonScroll = sonScroll
		this.setState({
			sonScroll
		})
	}
	...
	getSonActivityList = () => {
		...
		if (this.state.page[0] == -1) {
			status[0] = false
			Store.getInstance().photobaseHaveson.status = status
			this.setState({
				status
			})
			return false
		...
		Service.getSonActivityList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[0] = true
					loadingData[0] = true
					Store.getInstance().photobaseHaveson.dataList = dataList
					Store.getInstance().photobaseHaveson.page[0] = resp.data.next_page
					Store.getInstance().photobaseHaveson.status = status
		...
		Service.getPicList(opt).then((resp) => {
			...
			switch (resp.code) {
				case 0:
					...
					status[1] = true
					loadingData[1] = true
					Store.getInstance().photobaseHaveson.picList = picList
					Store.getInstance().photobaseHaveson.picHeigth = picHeigth
					Store.getInstance().photobaseHaveson.page[1] = resp.data.next_page
					Store.getInstance().photobaseHaveson.status = status
					_this.setState({
						picList,
						picHeigth,
	...
	navbarTap = (index,sliderOffset) => {
		Store.getInstance().photobaseHaveson.index = index
		Store.getInstance().photobaseHaveson.sliderOffset = sliderOffset
	
		this.setState({
			sliderOffset:Store.getInstance().photobaseHaveson.sliderOffset,
			index:Store.getInstance().photobaseHaveson.index,
	...
	gotoDetail = (e) => {
		...
		Store.getInstance().photobaseHaveson.toggleNav = true
		hashHistory.push({
			pathname: 'details',
			query: {
				rid
									...
									return (
												<div className='lists-item' key={index}>
													...
													<div className='compethaveson-bottom flex flex-space-around flex-ver-center'>
															<div className='bottom-btn' data-id={item.id} onClick={this.gotoNext.bind(this)}>全部征集</div>
